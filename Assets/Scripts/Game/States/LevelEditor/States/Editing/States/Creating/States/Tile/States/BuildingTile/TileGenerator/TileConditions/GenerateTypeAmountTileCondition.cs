    ...
    protected bool CheckGenerateTypeAmount(List<Vector2> gridPositions) {
        ...
        switch (generateType) {
            ...
            case GenerateType.UserGenerated:
                List<Vector2> userGeneratedGridPositions = gridPositions.FindAll(x => LevelEditorTileGrid.Instance.GetTile(x).UserGenerated);
                amount = userGeneratedGridPositions.Count;
                break;
            case GenerateType.AutoGenerated:
                List<Vector2> autoGeneratedGridPositions = gridPositions.FindAll(x => !LevelEditorTileGrid.Instance.GetTile(x).UserGenerated);
